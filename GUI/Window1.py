...
class a:
    ...
    def Window(self, window):
        self.button_Launch = QPushButton("启动游戏", window)
        self.button_Launch.setGeometry(1020, 640, 250, 90)  # 启动游戏按钮
        self.button_Launch.setStyleSheet(button_qss_1)
        self.button_Launch.clicked.connect(self.Launch_asyncio)  # 将启动游戏按钮绑定到launch槽

        self.button_LaunchSetup = QPushButton("启动设置", window)
...
def thead_open():
    ...
    os.system(JavaPath)  # 切换到java所在的路径
